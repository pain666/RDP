...
void write_num(int fd, uint16_t n) {
	uint8_t buf[] = { (n) & 0xFF, (n) >> 8 };
	write((fd), buf, 2);
}
...
{
    Node *node = (Node*)malloc(sizeof(*node) + degree * sizeof(Node *));
    if (node)
        node->key = key;
    return node;
}
...
{
    int i, r, g, b, v;
    ge_GIF *gif = (ge_GIF*)malloc(sizeof(*gif) + 2*width*height);
    if (!gif)
        goto no_gif;
    ...
    gif->fd = creat(fname, S_IWRITE/*0666*/);
    if (gif->fd == -1)
        goto no_fd;
#ifdef _WIN32
	...
	uint8_t buf[] = { 0xF0 | (depth - 1), 0x00, 0x00 };
    write(gif->fd, buf, 3);
    if (palette) {
...
{
	uint8_t buf[] = { '!', 0xFF, 0x0B };
	write(gif->fd, buf, 3);
    write(gif->fd, "NETSCAPE2.0", 11);
	uint8_t buf2[] = { 0x03, 0x01};
    write(gif->fd, buf2, 2);
